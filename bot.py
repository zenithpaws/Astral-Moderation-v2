...
async def handle_interaction(request):
    ...
    if request.method == 'POST':
        ...
        # Check if the payload contains an interaction
        if 'type' in payload and payload['type'] == 1:
            # Respond to the ping with a Pong
            return json.dumps({'type': 1})

        # Handle other interaction types (e.g., application commands)
        elif 'type' in payload and payload['type'] == 2:
            # Extract the command name and options
...
- `help` | Show help information.""")
    ...
    # Return a default response if no interaction is detected
    return json.dumps({'message': 'No interaction detected'})

# Define an HTTP route for handling interactions
@bot.route('/interactions', methods=['POST'])
async def interactions_route(request):
    # Call the handle_interaction function to process the request
    response = await handle_interaction(request)

    # Return the response
    return nextcord.web.Response(text=response, status=200, content_type='application/json')
