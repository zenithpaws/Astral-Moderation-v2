...
# Function to get the bot invite link
async def get_bot_invite(ctx):
    """Get the bot invite link from Firestore."""
    try:
        invite_ref = db.collection("secrets").document("bot_invite")
        snapshot = invite_ref.get()
        if snapshot.exists:
            return snapshot.to_dict().get("link")
        else:
...
# Function to set the bot invite link
async def set_bot_invite(invite_data):
    """Set the bot invite link in Firestore."""
    try:
        invite_ref = db.collection("secrets").document("bot_invite")
        invite_ref.set({"link": invite_data})
    except Exception as e:
...
# Command: Get bot invite link
@bot.slash_command(description="Get bot invite link")
async def botinvite(ctx):
    """Get bot invite link"""
    bot_invite = await get_bot_invite(ctx)
...
# Command: Set the bot invite link
@bot.slash_command(description="Set the bot invite link")
async def setbotinvite(ctx, invite: str):
    """Set the bot invite link"""
    if await permission_check(ctx):
        try:
            await set_bot_invite(invite)
            await ctx.send("Bot invite link has been set")
        except Exception as e:
            await ctx.send(f"Error setting bot invite link: {str(e)}")
...
def estimate_doc_size(doc_dict):
    """Recursively estimates the size of a document dictionary in bytes."""
    size = 0
    if isinstance(doc_dict, dict):
        for key, value in doc_dict.items():
            size += len(str(key).encode('utf-8'))
            size += estimate_doc_size(value)
    elif isinstance(doc_dict, list):
        for item in doc_dict:
            size += estimate_doc_size(item)
    elif isinstance(doc_dict, str):
        size += len(doc_dict.encode('utf-8'))
    elif isinstance(doc_dict, (int, float)):
        size += 8
    elif isinstance(doc_dict, bool):
        size += 1
    return size

@bot.slash_command(description="Check Firestore usage status and estimated storage size.")
async def databasestatus(ctx):
    """Calculates document counts, estimated storage usage, and Spark plan limits."""
    if await permission_check(ctx):
        await ctx.response.defer()  # Prevents slash command timeout while scanning
        ...
        try:
            ...
            # Get all top-level collections in your database
            ...
            for coll in collections:
                ...
                for doc in coll.stream():
                    coll_docs += 1
                    data = doc.to_dict() or {}
                    # Add document ID size + data payload size
                    doc_size = len(doc.id.encode('utf-8')) + estimate_doc_size(data)
                    coll_bytes += doc_size

                total_docs += coll_docs
                total_bytes += coll_bytes

                # Format collection-specific byte size
                coll_kb = coll_bytes / 1024
                collection_summary.append(f"• **{coll.id}**: {coll_docs} docs (`{coll_kb:.2f} KB`)")

            # Calculations for Free Tier Limits
            FREE_TIER_LIMIT_BYTES = 1024 * 1024 * 1024  # 1 GiB (1,073,741,824 bytes)
            used_mb = total_bytes / (1024 * 1024)
            percent_used = (total_bytes / FREE_TIER_LIMIT_BYTES) * 100

            # Build Embed Output
            embed = Embed(
                title="**Firebase Firestore Status**",
                color=nextcord.Color.orange()
            )
            
            embed.add_field(
                name="📊 Total Estimated Storage", 
                value=f"`{used_mb:.4f} MB` / `1,024 MB` ({percent_used:.4f}% of used)", 
                inline=False
            )
            embed.add_field(name="📄 Total Documents", value=f"`{total_docs}`", inline=True)
            
            if collection_summary:
                embed.add_field(
                    name="📁 Breakdown by Collection", 
                    value="\n".join(collection_summary), 
                    inline=False
                )

            embed.set_footer(text="Calculated live using standard Firebase Admin SDK")
            await ctx.send(embed=embed)
